...
// Rota para o endpoint GET que lê um carro pelo ID
router.get('/:id', (req, res) => {
  const carId = parseInt(req.params.id);
    ...
    const cars = JSON.parse(data); // Transforma o conteúdo em um objeto JavaScript
    const car = cars.find(car => car.id === carId); // Procura o carro pelo ID

    if (!car) {
      return res.status(404).json({ error: 'Carro não encontrado' });
    }

    res.json(car); // Retorna o carro encontrado como resposta
...
router.put('/:id', (req, res) => {
  ...
  fs.readFile(filePath, 'utf8', (err, data) => {
    ...
    let cars = JSON.parse(data); // Transforma o conteúdo em um objeto JavaScript
    let carIndex = cars.findIndex(car => car.id === carId); // Procura o índice do carro pelo ID
    ...
    fs.writeFile(filePath, JSON.stringify(cars), (err) => {
      ...
      res.json(cars[carIndex]); // Retorna o carro atualizado como resposta
    });
  });
});
